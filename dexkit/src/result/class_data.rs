...
impl<'a> ClassData<'a> {
    ...
    pub fn name(&self) -> Option<String> {
        self.get_dex_class().map(|dc| dc.class_name())
    }
    ...
    pub fn supper_class(&self) -> Option<ClassData<'a>> {
        let super_class = self.super_class.get_or_init(|| {
            self.super_class_id.and_then(|id| {
                let encode_id = Self::get_encode_id(self.dex_id, id);
                let list = self.bridge.get_class_by_ids(&vec![encode_id]);
                if !list.is_empty() {
                    Some(Box::new(list[0].clone()))
                } else {
                    None
                }
            })
        });
        super_class.as_deref().cloned()
    }
        ...
        self.interfaces
            .get_or_init(|| {
                let encode_ids: Vec<i64> = self
                    ...
                    .collect();
                self.bridge.get_class_by_ids(&encode_ids)
            })
            .clone()
    }
    ...
    pub(crate) fn with_meta(bridge: &'a DexkitBridge, meta: FBClassMeta<'a>) -> Self {
        ...
        let dex_class = OnceCell::new();
        let super_class = OnceCell::new();
        let interfaces = OnceCell::new();
        let methods = OnceCell::new();
        let fields = OnceCell::new();
        let annotations = OnceCell::new();
